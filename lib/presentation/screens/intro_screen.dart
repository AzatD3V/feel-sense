...
import 'package:xxx/presentation/screens/login_screen.dart';
    ...
    try {
      ...
      if (status.isGranted && mounted) {
        ...
        if (cameras.isEmpty && mounted) {
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(
              content: Text("Available Camera not founded",
                  textAlign: TextAlign.center)));
          return;
        }
        ...
        if (frontCamera == null && mounted) {
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(
              content: Text("Front Camera is not founded",
                  textAlign: TextAlign.center)));
        }
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(
              content:
                  Text('Camera access denied', textAlign: TextAlign.center)));
        }
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('Error 404', textAlign: TextAlign.center)));
      }
    }
  }
